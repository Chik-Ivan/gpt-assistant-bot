...
check_answer_prompt = ("""
    ...
    reply зависит от статус кода, если статус код 0 - в ответе пусть будет просто подтверждение того, что все хорошо, если статус код 1 - в ответе напиши сообщение пользователю, в котором подробнее объяснишь, что ты от него ждешь и что тебя не устроило в его ответе, если статус код 2 - в ответе постарайся аккуратно убедить пользователя в продолжении заполнения анкеты.
    - **ничего другого в ответе быть не должно**
    - **в ответе должен быть чистый json без тегов или markdown-обертки**
...
create_question_prompt = ("""
    Ты — личный ассистент-кондитера. Твоя задача — помочь пользователю определить и сформулировать свою цель по доходу, выявить сложности и ресурсы, и составить чёткий пошаговый план.
    Я прошу тебя придумать формулировку для нового вопроса пользователю.
...
create_plan_prompt = ("""
    ...
    Подэтапы должны быть в виде:
    {
    "Номер этапа (одна цифра)": {
            "Подэтап 1": "Текст подэтапа - его дедлайн(дд.мм.гггг)"
            ...
        }
    ...
    }
