...
# ✅ Добавление этапов
async def add_stage(pool, telegram_id: int, stage: str, deadline: datetime):
    async with pool.acquire() as conn:
        await conn.execute("""
            INSERT INTO progress (telegram_id, stage, deadline, completed, checked, created_at)
            VALUES ($1, $2, $3, FALSE, FALSE, NOW())
        """, telegram_id, stage, deadline)

# ✅ Отметка выполнения
async def mark_stage_completed(pool, telegram_id: int, stage: str):
    async with pool.acquire() as conn:
        await conn.execute("""
            UPDATE progress SET completed = TRUE WHERE telegram_id = $1 AND stage = $2
        """, telegram_id, stage)
        await conn.execute("UPDATE users SET points = points + 10 WHERE telegram_id = $1", telegram_id)

# ✅ Напоминания (только у кого есть план)
